...
def test_start_writes_task_status_and_dispatches(monkeypatch, tmp_repo):
    ...
    monkeypatch.setattr(
        ...
        lambda self, task: SimpleNamespace(adapter="multica", run_id="run-123", issue_id="AGO-77"),
    )
    ...
    assert status["executor_run"]["run_id"] == "run-123"
...
def test_start_uses_gate_override_when_provided(monkeypatch, tmp_repo):
    ...
    monkeypatch.setattr(
        "agos.cli.cmd_start.MulticaAdapter.start",
        lambda self, task: SimpleNamespace(adapter="multica", run_id="run-456", issue_id="AGO-88"),
    )
