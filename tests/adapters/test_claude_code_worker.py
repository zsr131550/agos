...
def test_sync_start_returns_completed_on_success(monkeypatch, tmp_path):
    ...
    assert calls[0][:4] == ["claude", "--safe-mode", "--permission-mode", "dontAsk"]
    assert "Do not ask clarifying questions" in calls[0][-1]


def test_sync_start_can_explicitly_bypass_permissions(monkeypatch, tmp_path):
    calls: list[list[str]] = []
    ...
    adapter = _adapter(tmp_path, dangerously_bypass_permissions=True)
