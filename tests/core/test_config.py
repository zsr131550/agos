...
def test_load_config(tmp_repo: Path) -> None:
    ...
    assert [g.id for g in feature.gates] == ["tests_pass", "no_secrets_in_diff", "build_clean"]


def test_resolve_gates_workflow(tmp_repo: Path) -> None:
    write_config(tmp_repo, SAMPLE)
    cfg = load_config(tmp_repo)
    gates = resolve_gates(cfg, "feature")
    assert [g.id for g in gates] == ["tests_pass", "no_secrets_in_diff", "build_clean"]


def test_resolve_gates_override(tmp_repo: Path) -> None:
    write_config(tmp_repo, SAMPLE)
    cfg = load_config(tmp_repo)
...
def test_resolve_gates_empty_workflow(tmp_repo: Path) -> None:
    write_config(tmp_repo, SAMPLE)
    cfg = load_config(tmp_repo)
    assert resolve_gates(cfg, "docs_only") == []


def test_resolve_gates_unknown_workflow(tmp_repo: Path) -> None:
    write_config(tmp_repo, SAMPLE)
    cfg = load_config(tmp_repo)
...
def test_gate_spec_command_xor_type() -> None:
...
def test_default_config_has_feature_workflow() -> None:
    cfg = default_config()
