...
def test_repo_paths_layout(tmp_repo: Path):
...
def test_git_head_returns_sha(tmp_repo: Path):
    head = git_head(tmp_repo)
    assert len(head) == 40
    assert all(c in "0123456789abcdef" for c in head)


def test_git_status_porcelain_empty(tmp_repo: Path):
    assert git_status_porcelain(tmp_repo) == ""


def test_git_status_porcelain_dirty(tmp_repo: Path):
    (tmp_repo / "new.txt").write_text("x", encoding="utf-8")
    out = git_status_porcelain(tmp_repo)
    assert "new.txt" in out


def test_find_repo_root_walks_up(tmp_repo: Path):
    (tmp_repo / ".agos").mkdir()
    nested = tmp_repo / "a" / "b"
...
def test_find_repo_root_none(tmp_path: Path):
    with pytest.raises(FileNotFoundError):
        find_initialized_repo_root(tmp_path)


def test_find_repo_root_requires_agos_when_git_only(tmp_path: Path):
    (tmp_path / ".git").mkdir()
    with pytest.raises(FileNotFoundError):
        find_initialized_repo_root(tmp_path)
